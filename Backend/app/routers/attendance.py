...
def validate_sign_in_status(status: AttendanceStatus) -> str:
    normalized_status = status.value if isinstance(status, AttendanceStatus) else str(status).lower()
    if normalized_status not in ATTENDED_STATUSES:
        raise HTTPException(400, "Sign-in status must be either 'present' or 'late'")
...
def classify_sign_in_status(event: Event, recorded_time: datetime) -> str:
    start_time = normalize_utc_naive(event.start_datetime)
    current_time = normalize_utc_naive(recorded_time)
    late_threshold_minutes = max(event.late_threshold_minutes or 0, 0)

    if current_time < start_time:
        return AttendanceStatus.PRESENT.value

    minutes_passed = int((current_time - start_time).total_seconds() // 60)
    if minutes_passed <= late_threshold_minutes:
        return AttendanceStatus.PRESENT.value
...
class StudentSelfCheckInRequest(BaseModel):
    event_id: int
    ...
    try:
        ...
        try:
            ...
            attendance_query = db.query(
                ...
                func.count(case((AttendanceModel.status.in_(ATTENDED_STATUSES), 1))).label('total_attended'),
...
def get_student_attendance_report(
    ...
    total_attended = len([a for a in attendances if a.status in ATTENDED_STATUSES])
    total_late = len([a for a in attendances if a.status == AttendanceStatus.LATE.value])
    ...
    for attendance in attendances:
        if attendance.event and attendance.event.start_datetime:
            ...
            if month_key not in monthly_stats:
                monthly_stats[month_key] = {"present": 0, "late": 0, "absent": 0, "excused": 0}
...
def get_attendance_summary(
    ...
    return {
        "summary": {
            ...
            "attendance_rate": round((((present_count + late_count) / total_records) * 100) if total_records > 0 else 0, 2),
...
def record_student_self_check_in(
    data: StudentSelfCheckInRequest = Body(...),
    current_user: UserModel = Depends(get_current_user),
    db: Session = Depends(get_db)
):
    """Allow the logged-in student to mark themselves present for an ongoing event."""
    if not current_user.student_profile:
        raise HTTPException(403, "Only students can check themselves in")

    event = get_event_or_404(db, data.event_id)

    now = datetime.utcnow()
    start_time = normalize_utc_naive(event.start_datetime)
    end_time = normalize_utc_naive(event.end_datetime)
    is_currently_running = start_time <= now <= end_time
    event_is_ongoing = (
        event.status == EventStatus.ONGOING
        or getattr(event.status, "value", None) == EventStatus.ONGOING.value
    )

    if not event_is_ongoing and not is_currently_running:
        raise HTTPException(400, "Attendance is only available for ongoing events")
    ...
    if existing:
        raise HTTPException(400, "Attendance already recorded for this event")
    ...
    attendance = AttendanceModel(
        ...
        time_in=now,
        method="manual",
        status=classify_sign_in_status(event, now),
        notes="Student self check-in"
    )
...
def record_face_scan_attendance(
    ...
    validate_sign_in_status(status)
    ...
    attendance = AttendanceModel(
        ...
        event_id=event_id,
        time_in=recorded_time,
        method="face_scan",
        status=classify_sign_in_status(event, recorded_time),
...
def record_manual_attendance(
    ...
    validate_sign_in_status(data.status)
    recorded_time = datetime.utcnow()
...
def record_bulk_attendance(
    ...
    for record in data.records:
        event = get_event_or_404(db, record.event_id)
        student = db.query(StudentProfile).filter(
        ...
        validate_sign_in_status(record.status)
        recorded_time = datetime.utcnow()
...
def mark_absent_no_timeout(
    ...
    attendances_to_update = db.query(AttendanceModel).filter(
        ...
        AttendanceModel.status.in_(ATTENDED_STATUSES)
    ).all()
