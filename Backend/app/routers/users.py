...
def get_all_users(
    ...
    # Defensive: remove any UserRole entries that have no associated Role to avoid Pydantic validation errors
    for u in users:
        try:
            u.roles = [ur for ur in u.roles if ur.role is not None]
        except Exception:
            # If roles can't be iterated for any reason, set to empty list
            u.roles = []
